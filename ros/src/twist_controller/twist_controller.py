...
class Controller(object):
    def __init__(self, vehicle_mass , wheel_radius, accel_limit , decel_limit , wheel_base, steer_ratio, min_speed , max_lat_accel , max_steer_angle):
        # TODO: Implement
        ...
        self.throttle_pid = PID(throttle_KP, throttle_KI, throttle_KD, mn=0.0, mx=1.0)
        self.steering_pid = PID(steering_KP, steering_KI, steering_KD, mn=-max_steer_angle, mx=max_steer_angle )
        self.filter = LowPassFilter(0.2, 0.1)

    def control(self, proposed_linear_velocity, proposed_angular_velocity, current_linear_vel, dbw_enabled):

    	# USE Yaw Controller to convert (and Low pass filter) to control
    	# implement Controller to control velocity
    	# Using a PID controller for high frequency signals (like noise) is disastrous due to Differential sections amplitude being tied directly to the frequency of the signal
        ...
        # Force Forward - vehicle_mass*acceleration (make sure it is decellerating)
        ...
        steering = self.yaw_controller.get_steering(proposed_linear_velocity, proposed_angular_velocity, current_linear_vel)
